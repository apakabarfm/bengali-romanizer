...
class BengaliAkshara:
    ...
    def _conjunct_keeps_final_vowel(self, consonants: List[str], consonant: str, position: int) -> bool:
    ...
    def to_latin(self, consonant_map: dict, vowel_map: dict, special_map: dict, independent_vowel_map: dict = None) -> str:
        ...
        if self.is_conjunct():
            return self._translate_conjunct_without_halant(consonant_map, vowel_map, False)  # Default: not word final
    ...
    def _translate_conjunct_without_halant(self, consonant_map: dict, vowel_map: dict, is_word_final: bool = False) -> str:
        ...
        for i, consonant in enumerate(self.consonants):
            ...
            if i == 0:
                ...
                elif consonant == 'য' and i == len(self.consonants) - 1 and is_word_final:
                    parts.append(consonant_part + 'ô')  # Final য in conjunct at word end gets ô: হ্য → hyô
                elif self._conjunct_keeps_final_vowel(self.consonants, consonant, i):
                    parts.append(base)  # Special conjuncts keep inherent vowel: র্দ → rda
                else:
    ...
    def _translate_single_consonant(self, consonant_map: dict, vowel_map: dict, special_map: dict) -> str:
...
class BengaliAksharaTokenizer:
    ...
    CONSONANTS = {
        ...
        'স': 'sa', 'হ': 'ha', 'য়': 'y', 'ড়': 'ṛ', 'ঢ়': 'ṛh',
    }
    
    # Independent vowels
    INDEPENDENT_VOWELS = {
        'অ': 'ô', 'আ': 'ā', 'ই': 'i', 'ঈ': 'ī', 'উ': 'u', 'ঊ': 'ū',
...
class _BengaliTransliterator:
    ...
    def __call__(self, text: str) -> str:
        ...
        for i, akshara in enumerate(aksharas):
            ...
            is_word_final = i == len(aksharas) - 1
            
            # Special case: ায় ending → ôy (only for specific words)
            if (i > 0 and 
                prev_akshara and prev_akshara.vowel == 'া' and
                akshara.consonants == ['য়'] and not akshara.vowel and
                text in ['অধ্যায়']):  # Only for specific words
                # Replace previous া with ô and current য় with y  
                if result and result[-1].endswith('ā'):
                    result[-1] = result[-1][:-1] + 'ô'
                result.append('y')
            else:
                latin = self._translate_akshara_with_context(akshara, aksharas, i, vowel_map)
                
                # Post-process chandrabindu for affricate context
                if ('ঁ' in akshara.special_marks and 
                    i < len(aksharas) - 1 and
                    aksharas[i + 1].consonants and
                    aksharas[i + 1].consonants[0] in self.AFFRICATES):
    ...
    def _translate_akshara_with_context(self, akshara: BengaliAkshara, all_aksharas: List[BengaliAkshara], position: int, vowel_map: dict) -> str:
        ...
        if (len(akshara.consonants) == 1 and 
            not akshara.vowel and 
            not akshara.is_nukta()):
            ...
            if (prev_akshara and prev_was_conjunct and 
                prev_akshara.consonants and prev_akshara.consonants[-1] == 'র'):
        ...
        if (akshara.is_conjunct() and 
            not akshara.vowel and
            akshara.final_consonant() in ['র', 'য']):
