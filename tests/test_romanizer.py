...
@pytest.mark.parametrize("bengali,expected", [
...
def test_bengali_syllable_tokenization():
    ...
    lexer = Lexer(text)
    print(f"\nLexer analysis:")
    print(f"  pos=0: peek()='{lexer.peek()}', peek(1)='{lexer.peek(1)}', peek(2)='{lexer.peek(2)}'")
    ...
    for case in test_cases:
        ...
        for i, syl in enumerate(syllables):
            print(f"  [{i}]: consonants={syl.consonants}, halant={syl.ending_halant}, vowel={syl.vowel}")
...
def test_bengali_akshara_translation_methods():
    ...
    assert akshara.needs_epenthetic_vowel(1) == True, "ক should need epenthetic vowel in ভক conjunct"
...
def test_paunche_chandrabindu():
    ...
    assert result == 'pauṅche', "পৌঁছে should have ṅ before affricate: pauṅche not paũche"
...
def test_affricate_detection_micro():
    ...
    assert is_affricate, f"ছ should be detected as affricate by peek_any, got: {lexer.peek()}"
...
def test_consonant_after_halant_conjunct():
    ...
    assert result == 'dhrm', "ধর্ম should be dhrm - consonant after halant conjunct loses vowel"
...
def test_dhrm_step_by_step():
    ...
    result1 = transliterator._translate_akshara_with_context(aksharas[0], aksharas, 0, transliterator.vowel_map)
    assert result1 == 'dhr', f"First akshara should be 'dhr': {result1}"
    
    result2 = transliterator._translate_akshara_with_context(aksharas[1], aksharas, 1, transliterator.vowel_map) 
...
def test_bengali_akshara_integration():
    ...
    for i, akshara in enumerate(aksharas):
        result = akshara.to_latin(consonant_map, vowel_map, special_map, tokenizer.INDEPENDENT_VOWELS)
...
def test_bengali_single_consonant_context():
    ...
    result_l = akshara_l._translate_single_consonant(consonant_map, vowel_map, special_map)
...
def test_context_detection_for_andolon():
    ...
    aksharas = [
        ...
        BengaliAkshara(['ন'], None, False, []),       # ন → should be 'n' (word continues after vowel)
    ]
    ...
    assert context_result_2 == True, "ল should detect vowel context from preceding নদো"
    
    # Test position 3: ন after ল (which followed vowel context)
    context_result_3 = transliterator._has_vowel_context(aksharas, 3)
    assert context_result_3 == True, "ন should detect vowel context propagated through word"